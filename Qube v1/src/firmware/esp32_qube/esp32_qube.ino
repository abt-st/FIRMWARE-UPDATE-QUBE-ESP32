...
void runHoming(float rawPos, float pendPos) {
  ...
  switch (homing_phase) {
    ...
    case H_TOUCH_NEG: {
      ...
      setMotorDirect((positive ? 1 : -1) * (fast ? HOMING_PWM_SEEK : HOMING_PWM_TOUCH));
  ...
  if ((nowUs - lastControlUs) >= CONTROL_PERIOD_US) {
    ...
    // MODO 5: Swing-up por energia con kick continuo
      ...
      if (spinning || inCooldown) {
        ...
        bool nearVertical = fabsf(pendPos) > swingupCatchDeg;
        ...
        bool atPeakTransition = atPeak && (180.0f - fabsf(pendPos) < (180.0f - swingupCatchDeg));
        ...
        bool forcedTransition = fabsf(pendPos) > fminf(swingupCatchDeg + 10.0f, 178.0f) &&
