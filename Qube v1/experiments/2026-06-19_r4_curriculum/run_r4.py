...
MLFLOW_EXPERIMENT = "qube_r4_curriculum"

# Best swing-up policy from R2 (kept for reference; R4 has no warm-start config).
WARM_START_MODEL = REPO_ROOT / "experiments/2026-06-18_r2_balance/models/r2_02_linear_alpha_control_s1.zip"

# R4 experiment matrix. R3 verdict: `linear_alpha_stabilise` killed the swing-up
# (0 % reach, the velocity penalty hits the high-speed pump through the top), but
# `linear_alpha` + reverse curriculum was the breakthrough — config 03 hit upright
# 30.4 % / hold 0.92 s (best ever, 0.08 s short of the 1 s balance threshold) on a
# single seed at 300k. R4 doubles down on the WINNING lever: drop stabilise, scale
# the curriculum config to 500k with 3 seeds (confirm it is not a single-seed fluke
# and see if more steps cross 1 s), plus a more aggressive curriculum branch (0.6).
# Order puts the proven 0.4 config first so 3 seeds finish before budget bites.
EXPERIMENTS: list[dict] = [
    # Proven winner (R3 config 03), now 500k x 3 seeds.
    {"name": "01_curriculum04", "reward": "linear_alpha",
     "potential": None, "near_upright_prob": 0.4, "base_model": None},
    # More near-upright starts: more balance practice, at some risk to swing-up reach.
    {"name": "02_curriculum06", "reward": "linear_alpha",
     "potential": None, "near_upright_prob": 0.6, "base_model": None},
]
