"""Finish R6: seed 0 last 100k from 400k checkpoint."""
...
def main() -> None:
    ...
    ckpt = str(REPORT_DIR / "models/r6_theta100_s0_step400000")
    log(f"Loading: {ckpt}")
    model = SAC.load(ckpt, env=env)

    models_dir = REPORT_DIR / "models"
    name = "theta100_s0"
    params = build_params(seed=0, reward="linear_alpha", potential="None",
    ...
    with mlflow_run(f"{name}_from400000", params, **mlflow_kw):
        log(f"  [{name}] 400,000 to 500,000")
    ...
    log(f"  Final model saved: r6_{name}.zip")
    ...
    log(f"  FINAL: balance={balance['balance_rate']*100:.1f}% "
        f"upright={balance['upright_fraction']*100:.1f}% "
        f"hold={balance['max_hold_s_best']:.2f}s")
