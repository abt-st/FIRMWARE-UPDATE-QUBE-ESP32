"""Continue R6: seed 1 from 350k, seed 2 from scratch."""
...
def train_seed(seed: int, remaining: int, start_step: int, ckpt: str | None = None) -> None:
    ...
    with mlflow_run(f"{name}_from{start_step}", params, **mlflow_kw):
        ...
        while done < remaining:
            ...
            log(f"  Checkpoint: r6_{name}_step{start_step + done}.zip")
    ...
    log(f"  [{name}] FINAL: balance={balance['balance_rate']*100:.1f}% "
...
def main() -> None:
    ...
    log("\n--- Seed 1: from 350k ---")
    try:
        train_seed(1, 150_000, 350_000, str(REPORT_DIR / "models/r6_theta100_s1_step350000.zip"))
