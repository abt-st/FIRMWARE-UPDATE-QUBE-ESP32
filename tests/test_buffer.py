...
@dataclass
class Sample:
    """Minimal sample for testing SignalBuffer."""
...
class TestSignalBuffer:
    ...
    def test_push_and_size(self) -> None:
        ...
        buf.push(Sample(timestamp=1.0))
        assert buf.size == 1
        buf.push(Sample(timestamp=2.0))
        assert buf.size == 2
    ...
    def test_get_all_returns_all(self) -> None:
        ...
        buf = SignalBuffer(maxlen=100)
        for i in range(5):
            buf.push(Sample(timestamp=float(i)))
        samples = buf.get_all()
    ...
    def test_elapsed(self) -> None:
        ...
        buf = SignalBuffer(maxlen=100)
        assert buf.elapsed() == 0.0
        buf.push(Sample(timestamp=10.0))
        assert buf.elapsed() == 0.0
        buf.push(Sample(timestamp=15.0))
