...
class SignalBuffer:
    ...
    def reset(self):
        with self._lock:
            for q in (self.t, self.position, self.setpoint, self.error,
                      self.pwm, self.current_ma, self.voltage_v, self.power_mw):
                q.clear()
            self._t0 = time.time()
    ...
    def export_csv(self, path: str):
        ...
        with open(path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([
                "t_s", "position_deg", "setpoint_deg", "error_deg",
                "pwm", "current_ma", "voltage_v", "power_mw"
            ])
            for row in zip(t, pos, sp, err, pwm, ima, vb, pmw):
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(top, text="IP ESP32:", bg=COLORS["panel"],
                 fg=COLORS["text"], font=("Consolas", 10)).pack(side="left")
        self._ip_var = tk.StringVar(value=ESP32Client.DEFAULT_IP)
        tk.Entry(top, textvariable=self._ip_var, width=14,
                 bg=COLORS["accent"], fg=COLORS["text"],
                 insertbackground="white",
                 font=("Consolas", 10)).pack(side="left", padx=(4, 10))

        tk.Label(top, text="Poll (ms):", bg=COLORS["panel"],
                 fg=COLORS["text"], font=("Consolas", 10)).pack(side="left")
        self._poll_var = tk.StringVar(value="100")
        tk.Entry(top, textvariable=self._poll_var, width=5,
                 bg=COLORS["accent"], fg=COLORS["text"],
                 insertbackground="white",
                 font=("Consolas", 10)).pack(side="left", padx=(4, 10))

        self._btn_connect = tk.Button(
            top, text="⚡ Conectar", command=self._toggle_connection,
            bg=COLORS["green"], fg="white", font=("Consolas", 10, "bold"),
            relief="flat", padx=10)
        self._btn_connect.pack(side="left", padx=(0, 8))

        self._lbl_status = tk.Label(
            top, text="● Sin conexión", bg=COLORS["panel"],
            fg=COLORS["red"], font=("Consolas", 10, "bold"))
        self._lbl_status.pack(side="left", padx=8)

        self._lbl_latency = tk.Label(
            top, text="latencia: —", bg=COLORS["panel"],
            fg=COLORS["text"], font=("Consolas", 9))
        self._lbl_latency.pack(side="left", padx=8)

        # Botón STOP siempre visible
        tk.Button(top, text="■ STOP", command=self._emergency_stop,
                  bg=COLORS["red"], fg="white",
                  font=("Consolas", 11, "bold"),
                  relief="flat", padx=12).pack(side="right", padx=4)

        tk.Button(top, text="⬛ Borrar buffer", command=self._clear_buffer,
                  bg=COLORS["accent"], fg=COLORS["text"],
                  font=("Consolas", 9), relief="flat", padx=8
                  ).pack(side="right", padx=4)

        tk.Button(top, text="💾 Exportar CSV", command=self._export_csv,
                  bg=COLORS["accent"], fg=COLORS["text"],
                  font=("Consolas", 9), relief="flat", padx=8
                  ).pack(side="right", padx=4)
    ...
    def _build_charts(self, parent):
        ...
        self._ln_pos, = self._ax_pos.plot([], [], color=COLORS["pos"],
                                          lw=1.5, label="Posición")
        self._ln_sp,  = self._ax_pos.plot([], [], color=COLORS["setpoint"],
                                          lw=1.2, ls="--", label="Setpoint")
        self._ln_err, = self._ax_pos.plot([], [], color=COLORS["error"],
                                          lw=1.0, label="Error")
        ...
        self._ax_pwm.set_title("Señal PWM (−255 … +255)", color=COLORS["text"],
                               fontsize=9, pad=4)
        ...
        self._ax_pwr.set_title("Potencia eléctrica (INA219)", color=COLORS["text"],
                               fontsize=9, pad=4)
        ...
        self._ln_ima, = self._ax_pwr.plot([], [], color=COLORS["current"],
                                          lw=1.5, label="Corriente (mA)")
        ...
        self._ln_vb, = self._ax_v.plot([], [], color=COLORS["voltage"],
                                       lw=1.2, ls="--", label="V bus")
        self._ax_pwr.legend(loc="upper left", fontsize=7,
                            facecolor=COLORS["panel"], labelcolor=COLORS["text"])
    ...
    def _build_control_panel(self, parent):
        ...
        def section(text):
            tk.Label(panel, text=text, bg=COLORS["accent"], fg=COLORS["text"],
                     font=("Consolas", 9, "bold"),
                     pady=3, padx=6, anchor="w").pack(fill="x", pady=(8, 2))
        ...
        for val, label in MODE_NAMES.items():
            tk.Radiobutton(
                f_mode, text=label, variable=self._mode_var, value=val,
                command=self._send_mode,
                bg=COLORS["panel"], fg=COLORS["text"], selectcolor=COLORS["accent"],
                font=("Consolas", 9), activebackground=COLORS["panel"]
            ).pack(anchor="w")

        # ── PWM Manual ────────────────────────────────────────────────
        section("PWM MANUAL (modo 1)")
        self._pwm_scale = tk.Scale(
            panel, from_=-255, to=255, orient="horizontal",
            resolution=1, command=self._on_pwm_scale,
            bg=COLORS["panel"], fg=COLORS["text"],
            troughcolor=COLORS["accent"], highlightthickness=0,
            font=("Consolas", 8))
        ...
        tk.Entry(f_sp, textvariable=self._sp_var, width=8,
                 bg=COLORS["accent"], fg=COLORS["text"],
                 font=("Consolas", 10), insertbackground="white"
                 ).pack(side="left")
        tk.Label(f_sp, text="°", bg=COLORS["panel"], fg=COLORS["text"],
                 font=("Consolas", 10)).pack(side="left", padx=2)
        tk.Button(f_sp, text="Enviar", command=self._send_setpoint,
        ...
        for label, var in (("Kp:", self._kp_var),
                           ("Ki:", self._ki_var),
                           ("Kd:", self._kd_var)):
            def _row(lbl=label, v=var):
                row(panel, lbl, lambda f, _v=v: tk.Entry(
                    f, textvariable=_v, width=8,
                    bg=COLORS["accent"], fg=COLORS["text"],
                    font=("Consolas", 9), insertbackground="white"
                ).pack(side="left"))
            _row()

        tk.Button(panel, text="Aplicar PID", command=self._send_pid,
                  bg=COLORS["yellow"], fg="#1a1a1a",
        ...
        tk.Label(f_cpr, text="CPR:", bg=COLORS["panel"], fg=COLORS["text"],
                 font=("Consolas", 9)).pack(side="left")
        self._cpr_var = tk.StringVar(value="2048")
        tk.Entry(f_cpr, textvariable=self._cpr_var, width=7,
                 bg=COLORS["accent"], fg=COLORS["text"],
                 font=("Consolas", 9), insertbackground="white"
                 ).pack(side="left", padx=4)
        tk.Button(f_cpr, text="Set", command=self._send_cpr,
                  bg=COLORS["accent"], fg=COLORS["text"],
        ...
        tk.Label(f_dir, text="Dir:", bg=COLORS["panel"], fg=COLORS["text"],
                 font=("Consolas", 9)).pack(side="left")
        self._dir_var = tk.IntVar(value=1)
        tk.Radiobutton(f_dir, text="+1", variable=self._dir_var, value=1,
                       command=self._send_dir,
                       bg=COLORS["panel"], fg=COLORS["text"],
                       selectcolor=COLORS["accent"], font=("Consolas", 9),
        ...
        tk.Button(panel, text="⬜ Zero aquí", command=self._zero_here,
                  bg=COLORS["accent"], fg=COLORS["text"],
        ...
        tk.Label(f_win, text="Mostrar:", bg=COLORS["panel"], fg=COLORS["text"],
                 font=("Consolas", 9)).pack(side="left")
        self._window_var = tk.StringVar(value="30")
        tk.Entry(f_win, textvariable=self._window_var, width=5,
                 bg=COLORS["accent"], fg=COLORS["text"],
                 font=("Consolas", 9), insertbackground="white"
                 ).pack(side="left", padx=4)
        tk.Label(f_win, text="s", bg=COLORS["panel"], fg=COLORS["text"],
                 font=("Consolas", 9)).pack(side="left")
    ...
    def _make_status_label(self, parent, text):
        lbl = tk.Label(parent, text=text, bg=COLORS["panel"],
                       fg=COLORS["text"], font=("Consolas", 9),
                       anchor="w", padx=10)
        lbl.pack(fill="x")
        return lbl
    ...
    def _animate(self, _frame):
        t, pos, sp, err, pwm, ima, vb, pmw = self.buffer.snapshot()
        if len(t) < 2:
            return
        ...
        t_w, pos_w, sp_w, err_w, pwm_w, ima_w, vb_w = (
            t[mask], pos[mask], sp[mask], err[mask],
            pwm[mask], ima[mask], vb[mask])
    ...
    def _refresh_labels(self, s: QubeState):
        ...
        self._lbl_latency.config(
            text=f"latencia: {self.client.last_latency_ms:.0f} ms")
        self._lbl_mode.config(
            text=f"Modo:    {MODE_NAMES.get(s.mode, s.mode)}")
        self._lbl_pos.config(
            text=f"Pos:     {s.position_deg:+.2f}°")
        self._lbl_sp_disp.config(
            text=f"Setpnt:  {s.setpoint_deg:+.2f}°")
        self._lbl_err.config(
            text=f"Error:   {s.error_deg:+.2f}°")
        self._lbl_pwm_d.config(
            text=f"PWM:     {s.pwm}")
        self._lbl_ima_d.config(
            text=f"I:       {s.i_ma:.1f} mA")
        self._lbl_vb_d.config(
            text=f"V bus:   {s.v_bus:.2f} V")
        self._lbl_pw_d.config(
            text=f"Potencia:{s.p_mw:.1f} mW")
