...
def analyze_csv(path):
    ...
    return {
        ...
        "max_servo": round(servo_max, 1),
        "lqr_catches": lqr_catches,
        "max_hold": round(max_hold, 1),
        "crash": crash,
        "duration": round(float(rows[-1].get("t", 0)), 1),
    }
...
for lo, hi in ranges:
    ...
    if count > 0:
        print(
            "  %3d-%3d: %3d ensayos, %2d catches (%d%% catch rate)" % (lo, hi, count, catch_in, catch_in * 100 // count)
        )
    else:
