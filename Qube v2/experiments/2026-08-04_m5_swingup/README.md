...
## 6. De dónde sacar esa energía: el bombeo está saturado el 92% del tiempo

La pregunta que dejó abierto el §5 —cómo subir la energía para llegar a 165°— se contesta
sin banco, sobre estas mismas trazas. `scripts/m5_pwm_sat.py`, corrido sobre las diez:

| | mín | mediana | máx |
|---|---|---|---|
| fracción del bombeo con el PWM en su techo | 86,7% | **92,5%** | 94,1% |

**El bombeo pide más PWM del que tiene permitido casi todo el tiempo.** Eso descarta de
entrada los dos mandos que parecían candidatos:

- **`?pr=`** (tope de la referencia de bombeo, hoy 70°) no puede hacer nada: agranda el
  error de seguimiento, y el error ya es lo bastante grande como para saturar. Peor, manda
  el brazo más lejos del centro, donde la atenuación por posición **baja** el techo.
- **`?ke=`** tampoco, por la misma razón —y además por [P23](../../docs/REGISTRO_PROBLEMAS.md#p23),
  que es un defecto independiente.

**Esto confirma [P11](../../docs/REGISTRO_PROBLEMAS.md#p11), que hasta hoy era una deducción
por lectura del código.** Queda medido.

### La prueba ingenua daba 0% y decía lo contrario

Contar `|pwm| >= sp` da **0,0%** de saturación en las diez corridas, con un `|pwm|` medio de
53–56 sobre un tope de 60. Leído así, parecía que sobraba autoridad.

Es un artefacto. Lo que el DAQ registra es `lastPwmCmd`, asignado dentro de `setMotorDirect`
(`esp32_qube.ino:1425`), o sea **después** de la atenuación por posición de `setMotor`
(`:1447-1451`):
...
El bombeo pide `sp` y al puente H llega `int(sp · factor)`. El valor exacto `sp` sólo
saldría con el brazo en θ = 0, así que comparar contra `sp` da 0% **siempre**, en cualquier
corrida, midiera lo que midiera. El techo hay que recalcularlo por muestra con el θ de la
propia traza; el script imprime las dos cifras lado a lado para que la diferencia se vea.

> Es el mismo error de forma que P6 (sobrepaso mal normalizado) y que el `c1 >= 4` del §5:
> un número que sale de la instrumentación y no del equipo. Van tres en este experimento.

### Lo que sí queda como candidato, y lo que no

La atenuación por posición **no** es el cuello: el techo efectivo medio es 58,1–59,1 sobre
60, porque el brazo trabaja centrado. El centro de oscilación va de −6,8° a +16,0° (mediana
5,4°), así que **`?pc=` no tiene nada que corregir** — los "−18 a −36°" de deriva que cita
el comentario de `:3976-3977` no se reproducen hoy.

Queda un solo mando capaz de darle más energía al bombeo: **`sp`**, el techo mismo. Y su
barrido histórico (`:1064-1071`: 50 → 70, óptimo en 60) es **anterior a P22**, o sea que se
midió sobre la referencia de α que derivaba — el mismo motivo por el que no son atribuibles
los barridos de `ke` y de `bt`. Hay que rehacerlo.

Con una tensión real que hay que vigilar: a `sp` = 70 el modo de fallo registrado era el
brazo cruzando los 95°, y con el bombeo sano el margen bajó de 26,7° a **14,9°**.
