...
# v4 (P19) adds ``seq`` / ``age`` / ``md`` so a stale or repeated observation can be
# detected instead of silently becoming training data. See ``_check_freshness``.
EXPECTED_RL_PROTO = 4

# ── P19: what counts as a usable observation ────────────────────────────────────
# Before this, ``step()`` caught the network error, logged a warning and KEPT THE LAST
# STATE. A dead episode — link down, mode changed under us, control loop stalled —
# produced observations that looked perfectly valid, and every downstream number was
# computed over them. That is a data-integrity defect, not a performance one.
#
# ``MAX_OBS_AGE_MS`` is generous on purpose: the firmware recomputes the observation at
# 50 Hz (20 ms), so 200 ms is ten missed ticks. Anything older is not "a bit late", it
# is a loop that stopped.
MAX_OBS_AGE_MS = 200
# How many identical ``seq`` values in a row before we call it frozen. One repeat can
# happen legitimately if the client polls faster than the firmware's 50 Hz tick.
MAX_REPEATED_OBS = 3

# ── P20: la frecuencia alcanzada es parte del contrato, no una sorpresa ─────────
# El lazo RL por HTTP corre a ~26 Hz, no a los 50 para los que se entrena, y nada lo
# decía: se entrenó y se evaluó sobre un enlace que entrega la mitad de los pasos.
# Una política de 50 Hz sobre un enlace de 26 ve una dinámica que no es la suya.
MIN_FREQ_RATIO = 0.8


class StaleObservationError(RuntimeError):
    """The board returned an observation that must not be trained on.

    Raised instead of degrading silently: an episode built on stale data is worse than
    no episode, because it is indistinguishable from a real one after the fact.
    """
...
class QubeRealEnv(gym.Env):
    ...
    def __init__(
        ...
        # P20: abortar si la frecuencia alcanzada cae por debajo de MIN_FREQ_RATIO de
        # la pedida. Por defecto ACTIVO: medir sobre un enlace que entrega la mitad de
        # los pasos no evalúa la política, y durante campañas enteras nada lo dijo.
        # `strict_freq=False` lo degrada a advertencia para trabajo exploratorio.
        ...
        self._zero_epoch = 0
        # P19: procedencia de la última observación aceptada.
        self._last_obs_seq: int | None = None
        self._repeated_obs = 0
        # P20: la frecuencia REAL alcanzada, medida por episodio en vez de asumida.
        self._steps_this_episode = 0
        self._episode_start_s = 0.0
        self._measured_hz: float | None = None
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        # P19: NO se conserva el último estado ante un fallo. Antes esto era
        # ``except RequestException: logger.warning(...)`` y el episodio seguía con el
        # número anterior — un enlace caído producía una traza que, leída después, era
        # indistinguible de una corrida real. Se prefiere un episodio que muere
        # ruidosamente a uno que miente en silencio.
        try:
            data = self._rl_step(self._action_sign * float(action[0]))
        except requests.RequestException as exc:
            raise StaleObservationError(
                "rl_step failed: the link is down. The episode ends here instead of "
                "continuing on the last known state (P19)."
            ) from exc
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None) -> tuple[np.ndarray, dict]:
        ...
        # P20: cerrar el episodio anterior midiendo la frecuencia que REALMENTE se
        # alcanzó, en vez de suponer `control_freq`. Se entrenó a "50 Hz" sobre un
        # enlace de 26,1 Hz durante campañas enteras sin que nada lo dijera: la
        # política ve una dinámica al doble de lento de la que aprendió.
        if self._steps_this_episode > 0 and self._episode_start_s > 0.0:
            elapsed = time.time() - self._episode_start_s
            if elapsed > 0.0:
                self._measured_hz = self._steps_this_episode / elapsed
                info["measured_hz"] = self._measured_hz
                ratio = self._measured_hz / float(self.timing.control_freq)
                if ratio < MIN_FREQ_RATIO:
                    logger.error(
                        "el lazo corrió a %.1f Hz contra los %.1f Hz pedidos (%.0f%%): "
                        "la política ve una dinámica que no es la que entrenó. P20.",
                        self._measured_hz,
                        self.timing.control_freq,
                        100 * ratio,
                    )
                    if self.strict_freq:
                        raise StaleObservationError(
                            f"frecuencia alcanzada {self._measured_hz:.1f} Hz < "
                            f"{MIN_FREQ_RATIO:.0%} de {self.timing.control_freq} Hz. "
                            "Medir con este enlace no evalúa la política (P20); "
                            "`strict_freq=False` lo degrada a advertencia."
                        )
        self._steps_this_episode = 0
    ...
    def _check_freshness(self, data: dict[str, Any], expected_mode: int = 6) -> None:
        """Reject an observation that is repeated, stale, or from another mode (P19).

        The firmware stamps every recomputation with a monotonic ``seq``, an ``age`` in
        ms and the ``md`` it ran in. Any of the three going wrong means the number in
        hand is not a measurement of what we think we are measuring.
        """
        seq = data.get("seq")
        age = data.get("age")
        md = data.get("md")
        if seq is None or age is None or md is None:
            raise StaleObservationError(
                "ESP32 /rl_step lacks seq/age/md: firmware predates the P19 fix. "
                "Reflash before running a campaign — without these fields a dead "
                "episode is indistinguishable from a live one."
            )

        if int(md) != expected_mode:
            raise StaleObservationError(
                f"observation was computed in mode {md}, not {expected_mode}. The board "
                "left the RL mode mid-episode (servo limit, watchdog or safety gate); "
                "everything after this point would be measuring nothing."
            )

        if int(age) > MAX_OBS_AGE_MS:
            raise StaleObservationError(
                f"observation is {age} ms old (limit {MAX_OBS_AGE_MS} ms). The 50 Hz "
                "control loop is not recomputing it — this is a stopped loop, not lag."
            )

        if seq == self._last_obs_seq:
            self._repeated_obs += 1
            if self._repeated_obs >= MAX_REPEATED_OBS:
                raise StaleObservationError(
                    f"the same observation (seq={seq}) came back {self._repeated_obs} "
                    "times in a row: /rl_state is frozen. This is exactly the failure "
                    "that made previous mode-6 campaigns unattributable."
                )
        else:
            self._repeated_obs = 0
            self._last_obs_seq = seq
